...
function simple_login_notification_email($login, $role) {
	...
	$exclude_admin = (isset($options['admin_email']) && !empty($options['admin_email'])) ? true : false;
...
function simple_login_notification_section_general() {
	...
	echo '<p>'. esc_html__('This plugin sends a detailed email notification when users log in to your site.', 'simple-login-notification');
...
function simple_login_notification_dismiss_notice_link() {
	...
	return '<a class="sln-dismiss-notice" href="'. esc_url($href) .'">'. esc_html($label) .'</a>';
